...
const NestedMenu: React.FC<{ menuItems: any }> = ({ menuItems }) => {  //MenuItem[]
  ...
  const activeStates = useMemo(() => {
    ...
    const calculateActive = (items: MenuItem[]) => {
      if (!items) return;
      items.forEach(item => {
        states[item.id] = item.link === router.pathname || (item.submenu ? hasActiveLink(item.submenu) : false);
        if (item.submenu) {
          calculateActive(item.submenu);
        }
      });
    };
    calculateActive(menuItems);
    return states;
  }, [router.pathname, menuItems, hasActiveLink]);
